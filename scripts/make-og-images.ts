...
function svgFor(title: string, subtitle: string): string {
    ...
    <text x="60" y="${H / 2 - 40}" font-family="monospace" font-size="${titleFs}" fill="#cecece" font-weight="700">${escapeHtml(title)}</text>
    <text x="60" y="${H / 2 + 40}" font-family="monospace" font-size="36" fill="#40b567">${escapeHtml(subtitle)}</text>
...
async function gen(outName: string, title: string, subtitle: string) {
	...
	const buf = await sharp(Buffer.from(svgFor(title, subtitle)))
		.png()
		.toBuffer();
...
for (const [path, entry] of Object.entries(m.flatIndex)) {
	...
	await gen(
		ogSlug(path),
		entry.entity.name,
		LABEL[entry.entity.programType] ?? "",
	);
	count++;
}
